...
export class RemoteState {
  public readonly createPullRequestUrlTemplate = new LocalStorage<string>(
    LocalStorageKey.CreatePullRequestUrlTemplate,
    "",
    this.path
  )
  ...
  public getCreatePullRequestUrl(branch: BranchSummaryBranch): string | null {
    ...
    return "" !== template ? template.replace("{{source}}", branch.name) : null
  }
}
