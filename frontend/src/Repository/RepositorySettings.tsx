...
export const RepositorySettings = observer(
  ({ repository }: RepositoryProps): ReactElement => {
    return (
      <Hidden label="Settings">
        <div>
          <LocalStorageInput
            title="Create pull request url template"
            storage={repository.remoteState.createPullRequestUrlTemplate}
          />

          <LocalStorageInput
            title="Jira path"
            storage={repository.timeTrackerState.jiraProvider.pathStorage}
          />
          <LocalStorageInput
            title="Jira user"
            storage={repository.timeTrackerState.jiraProvider.userStorage}
          />
          <LocalStorageInput
            title="Jira token"
            storage={repository.timeTrackerState.jiraProvider.tokenStorage}
          />
          ...
          />
          <Checkbox label="Push on commit" flag={repository.pushOnCommit} />
          <Checkbox
