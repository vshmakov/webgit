  ...
  ({ repository }: RepositoryProps): ReactElement => {
    ...
    const files = status.files
      ...
      .sort((file1: FileStatusResult, file2: FileStatusResult): number => {
        ...
        return (
          -compare(
            status.conflicted.indexOf(path1),
            status.conflicted.indexOf(path2)
          ) || compareAlphabetically(path1, path2)
        )
      })
      .map(
        (file: FileStatusResult): ReactElement => (
          <File repository={repository} file={file} key={file.path} />
        )
      )
