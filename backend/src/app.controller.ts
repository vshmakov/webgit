...
import {
  Response,
  Response as GitResponse,
} from 'simple-git/typings/simple-git';
...
async function getCachedStatus(
  ...
  if (
    null === cached.checking &&
    Date.now() - cached.lastStartedAt >= STATUS_INTERVAL
  ) {
...
export class AppController {
  ...
  public async jiraIssueSummary(@Query() query: IssueQuery): Promise<string> {
    const response = await fetch(
      `${query.path}/rest/api/latest/issue/${query.key}`,
      {
        headers: {
          Authorization:
            'Basic ' +
            Buffer.from(`${query.user}:${query.token}`).toString('base64'),
  ...
  public async youtrackIssueSummary(
    @Query() query: IssueQuery,
  ): Promise<string> {
    const response = await fetch(
      `${query.path}/api/issues/${query.key}?fields=summary`,
      {
        headers: {
  ...
  public async rebaseCurrentToBranch(
    @Headers() headers: PathHeaders,
    @Body() branch: BranchSummaryBranch,
  ): Promise<void> {
  ...
  @Put('/file/decline')
  public async declineFile(
    @Headers() headers: PathHeaders,
    @Body() file: FileStatusResult,
  ): Promise<void> {
    const client = git(headers);
